public class Book implements Comparable{
  ...
  public boolean equals(Book otherBOOK){
    if((this.author).compareTo(otherBOOK.getAuthor()) == 0){
      return true;
    }else{
      return false;
    }
  ...
  public int compareTo(Object other){
    Book otherBook = (other instanceof Book) ? (Book) other : new Book();
    if((this.title).compareTo(otherBook.getTitle()) == 0){
      return 0;
    }else if((this.title).compareTo(otherBook.getTitle()) < 0){
      return -1;
    }else{
      return 1;
    }
